...
| `orderBy` | string |  | The order of the events returned in the result. |
...
| `singleEvents` | string |  | Whether to expand recurring events into instances and only return single one-off events and instances of recurring events, but not the underlying recurring events themselves.  The default is False. Optional. |
